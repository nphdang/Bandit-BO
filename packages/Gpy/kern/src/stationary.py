...
class Matern52_Discrete(Stationary):
    """
    Matern 5/2 kernel with rounding:

    .. math::

       k(r) = \sigma^2 (1 + \sqrt{5} r + \\frac53 r^2) \exp(- \sqrt{5} r)
    """
    def __init__(self, input_dim, discrete_dim, variance=1., lengthscale=None, ARD=False, active_dims=None, name='Mat52_Discrete'):
        self.discrete_dim = discrete_dim  # get dim of discrete variable
        super(Matern52_Discrete, self).__init__(input_dim, variance, lengthscale, ARD, active_dims, name)

    def to_dict(self):
        input_dict = super(Matern52_Discrete, self)._to_dict()
        input_dict["class"] = "GPy.kern.Matern52_Discrete"
        return input_dict

    @staticmethod
    def _from_dict(kernel_class, input_dict):
        useGPU = input_dict.pop('useGPU', None)
        return Matern52_Discrete(**input_dict)
    ...
    def K(self, X, X2=None):
        ...
        """
        # round X
        X_round = np.array([np.append(np.around(x_val[:self.discrete_dim], 0), x_val[self.discrete_dim:]) for x_val in X])
        # round X2
        if X2 is not None:
            X2_round = np.array([np.append(np.around(x_val[:self.discrete_dim], 0), x_val[self.discrete_dim:]) for x_val in X2])
        else:
            X2_round = None
        r = self._scaled_dist(X_round, X2_round)
        # r = self._scaled_dist(X, X2)
        return self.K_of_r(r)
...
class Matern52_Categorical(Stationary):
    """
    Matern 5/2 kernel with one-hot encoding:

    .. math::

       k(r) = \sigma^2 (1 + \sqrt{5} r + \\frac53 r^2) \exp(- \sqrt{5} r)
    """
    def __init__(self, input_dim, categorical_dim, variance=1., lengthscale=None, ARD=False, active_dims=None, name='Mat52_Categorical'):
        self.categorical_dim = categorical_dim # get dim of categorical variable
        super(Matern52_Categorical, self).__init__(input_dim, variance, lengthscale, ARD, active_dims, name)

    def to_dict(self):
        input_dict = super(Matern52_Categorical, self)._to_dict()
        input_dict["class"] = "GPy.kern.Matern52_Categorical"
        return input_dict

    @staticmethod
    def _from_dict(kernel_class, input_dict):
        useGPU = input_dict.pop('useGPU', None)
        return Matern52_Categorical(**input_dict)

    # transform real-value vector to one-hot encoding
    def transform_to_one_hot(self, vector):
    ...
    def K(self, X, X2=None):
        ...
        """
        # transform X
        X_categorical = np.array([np.append(self.transform_to_one_hot(x_val[:self.categorical_dim]),
                                            x_val[self.categorical_dim:]) for x_val in X])
        # transform X2
        if X2 is not None:
            X2_categorical = np.array([np.append(self.transform_to_one_hot(x_val[:self.categorical_dim]),
                                                 x_val[self.categorical_dim:]) for x_val in X2])
        else:
            X2_categorical = None
        r = self._scaled_dist(X_categorical, X2_categorical)
        # r = self._scaled_dist(X, X2)
        return self.K_of_r(r)
...
class ExpQuad_Discrete(Stationary):
    """
    The Exponentiated quadratic covariance function with rounding.
    ...
    """
    def __init__(self, input_dim, discrete_dim, variance=1., lengthscale=None, ARD=False, active_dims=None, name='ExpQuad_Discrete'):
        self.discrete_dim = discrete_dim  # get dim of discrete variable
        super(ExpQuad_Discrete, self).__init__(input_dim, variance, lengthscale, ARD, active_dims, name)

    def to_dict(self):
        input_dict = super(ExpQuad_Discrete, self)._to_dict()
        input_dict["class"] = "GPy.kern.ExpQuad_Discrete"
        return input_dict

    @staticmethod
    def _from_dict(kernel_class, input_dict):
        useGPU = input_dict.pop('useGPU', None)
        return ExpQuad_Discrete(**input_dict)
...
class ExpQuad_Categorical(Stationary):
    """
    The Exponentiated quadratic covariance function with one-hot encoding.
    ...
    """
    def __init__(self, input_dim, categorical_dim, variance=1., lengthscale=None, ARD=False, active_dims=None, name='ExpQuad_Categorical'):
        self.categorical_dim = categorical_dim  # get dim of categorical variable
        super(ExpQuad_Categorical, self).__init__(input_dim, variance, lengthscale, ARD, active_dims, name)

    def to_dict(self):
        input_dict = super(ExpQuad_Categorical, self)._to_dict()
        input_dict["class"] = "GPy.kern.ExpQuad_Categorical"
        return input_dict

    @staticmethod
    def _from_dict(kernel_class, input_dict):
        useGPU = input_dict.pop('useGPU', None)
        return ExpQuad_Categorical(**input_dict)
